...
describe('Word search in mobile mode', () => {
  ...
  it('shows a list of 6 words', () => {
    cy.get('.game--word').should('have.length', 6);
  });

  it('keeps track of the user\'s progress', () => {
    cy.get('.game--progress').should('have.text', '0 / 6 found');

    getPath().then(path => {
      cy.get('.game--table').drawPath(path);
    });

    cy.get('.game--progress').should('have.text', '1 / 6 found');
  });
